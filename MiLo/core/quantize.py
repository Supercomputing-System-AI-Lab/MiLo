...
def full_to_int3_symmetric(tensor_in, LoRC_group_size):
...
class HQQMatmulNoCacheMul(torch.autograd.Function): # default
    @staticmethod
...
class HQQLinear(nn.Module):
    ...
    def quantize(
        ...
        if rank > 0:
            if lorc_dtype == 'int8':
                ...
                UV_quantized = ((full_to_int3_symmetric(U, 64)), (full_to_int3_symmetric(V, 64)))
                self.UV_quantized = UV_quantized
            else:
    ...
    def matmul(self, x: Tensor, transpose: bool = True) -> Tensor:
        ...
        if self.U != None and self.V != None:
            ...
            # if 'experts' in self.name:
            #     with open('/u/bhuang4/mixtral_offloading/HQQ_LoRC/expert_freq.txt', 'a') as f:

            #         f.write(f"{self.name[6:-3]}\n")
