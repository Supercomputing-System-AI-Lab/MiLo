...
//     iters = (group_blocks / thread_k_blocks) * ceildiv(iters, (group_blocks / thread_k_blocks)); //4,
  ...
  auto matmul = [&] (int k_mod_2) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      scale_with_zeros(frag_b0, frag_s[k_mod_2][j], 0);
      frag_b1 = dequant_with_zeros(b_quant_shift);
      scale_with_zeros(frag_b1, frag_s[k_mod_2][j], 1);
      #pragma unroll
      for (int i = 0; i < thread_m_blocks; i++) {
    ...
    scale_with_zeros(frag_b0, frag_s[k_mod_2][3], 0);
    frag_b1 =  dequant_with_zeros(b_quant_shift);
    scale_with_zeros(frag_b1, frag_s[k_mod_2][3], 1);
    #pragma unroll
    for (int i = 0; i < thread_m_blocks; i++) {
