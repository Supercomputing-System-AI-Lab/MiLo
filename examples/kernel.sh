...
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 <batchsize> <hidden_dimension> <mlp_intermediate_dimension> <tile_shape> <save_dir>"
    exit 1
fi
...
echo "===== MiLo kernel end-to-end Latency  ====="
python utils/MiLo_kernel_end_to_end_latency.py --base_dir ${save_dir} --model_id Mixtral
